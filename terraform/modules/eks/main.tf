...
resource "aws_eks_cluster" "main" {
  ...
  vpc_config {
    ...
    endpoint_public_access  = var.endpoint_public_access # VIOLATION: Pattern 28 - public endpoint enabled
    security_group_ids      = [data.aws_security_group.eks_cluster.id]
  }

  # VIOLATION: Pattern 29 - No audit logging enabled
  logging {
    cluster_logging = [
      {
        enabled = false
...
data "aws_security_group" "eks_cluster" {
  filter {
    name   = "tag:Name"
    values = ["${var.environment}-eks-cluster-sg"]
  }
}

# Optional: Add conditional node group for spot instances
